...
fn main() {
    ...
    println!("trained: loss={:.4} best={:.4}", stats.loss, stats.best_loss);
    ...
    let report = engine.dream(&mut mind, &[1, 2, 3], &mut rng).expect("dream");
    println!("dream best: {:?} (score {:.4})", report.best_ids, report.best_score);
